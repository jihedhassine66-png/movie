            ...
            <div className="filter-form">
                ...
                <form onSubmit={handleSubmit}>
                    <div style={{marginBottom:10}}>
                        ...
                        <input className="form-control" placeholder='1.0' type="number" step="0.1" min="0" max="10" value={minRating} onChange={(e) => setMinRating(e.target.value)} style={{width:'100%'}} />
                    </div>
                    <div style={{marginBottom:10}}>
                        <label style={{display:'block', fontSize:12, color:'var(--muted)'}}>Max rating</label>
                        <input className="form-control" placeholder='10.0' type="number" step="0.1" min="0" max="10" value={maxRating} onChange={(e) => setMaxRating(e.target.value)} style={{width:'100%'}} />
                    </div>
                    <div style={{display:'flex', justifyContent:'flex-end', gap:8}}>
                        <button type="button" onClick={() => setShowFilterForm(false)} className="btn btn-outline-light">Cancel</button>
                        <button type="submit" className="btn btn-success" style={{backgroundColor:'red',borderColor:'red',borderRadius:' 50px '}}>Apply</button>
                    </div>
                </form>
            </div>
        </div>
